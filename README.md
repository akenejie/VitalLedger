...
※ 本データベースはまだ検証が不十分です。また、フロントエンドが制作できていません。

**「お金」と「栄養」を一つの流れとして統合管理する、究極のライフログ・データベース**

このプロジェクトは、単なる家計簿でも、単なる冷蔵庫の在庫管理でも、単なるダイエット記録でもありません。
「通貨としてのポイント」「冷蔵庫の在庫」「摂取した栄養素」「消費税の端数処理」に至るまで、現実世界の商習慣と食生活を忠実に再現することを試みるデータベースです。

## 特徴とこだわり

既存の家計簿アプリや栄養管理アプリに不満を感じたことはありませんか？
本データベースは、以下の **5つの革新的な設計思想** によって、これまでのアプリでは手の届かなかった厳密な管理を実現します。
...
1. **Purchase**: レシート入力で `FRIDGE`（冷蔵庫）や `PANTRY`（保存庫）へ同時入庫。
...
本データベースには、「買い物時にどのポイントカードを提示したか」、という情報を、会計時に**どの還元権利を行使したか**として記録します。
多くのソフトウェアは「クレジットカード払い」を「借金（負債）」として扱いますが、本データベースではクレジットカードを財布として取り扱いません。**銀行口座からの即時引き落とし（デビット的扱い）** + **還元権利の行使**として一般化された形で記録します。そのため、利用可能な金額を計算するために「銀行口座の残高からクレジットカード利用分を引き算する」作業は不要になります。
...
* **`m_foods_measured`**: 調味料など、重量(g)で管理する食品。
...
決済手段（クレジットカードやポイントカード提示）を「権利の行使」として一般化し、ポイント発生のロジックを管理します。

* **`m_reward_rules`**: 「200円で1pt」「100万円で1万pt」「月間累計」「年間累計」といった計算ロジックの定義。期間集計ロジック（`period_type`）により、端数の扱いを厳密に制御します。
* **`m_reward_source_wallets`**: どの財布（口座）からの支払いが、どの還元ルールの対象になるかの紐付け。
* **`t_reward_logs`**: 「ある支払いがトリガーとなり、あるルールが行使され、結果としてポイントが発生した」という事実のリンクテーブル。
