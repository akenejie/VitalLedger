...
このプロジェクトは、単なる家計簿でも、単なるダイエットアプリでもありません。
「通貨としてのポイント」「冷蔵庫の在庫」「摂取した栄養素」「消費税の端数処理」に至るまで、現実世界の物理と商習慣を忠実にモデリングしたデータベースです。

## 特徴とこだわり

既存の家計簿アプリや栄養管理アプリに不満を感じたことはありませんか？
本システムは、以下の **4つの革新的な設計思想** によって、これまでのアプリでは手の届かなかった厳密な管理を実現します。
...
* **商品合計と支払合計が一致しない**（高還元率のポイントバック、割引クーポンの複雑な適用）
...
* 全ての資金移動データに `expiry_at`（有効期限）と `usage_restriction`（用途限定）のカラムを実装。
* 「来月切れる期間限定のお金を使って支払う」という消込処理を追跡可能。

### 3. 日本の商習慣に即した消費税計算

「商品ごとに税計算して合算」では、実際のレシートと1円〜数円のズレが生じることがあります。
本データベースは **「取引全体の商品を合算してから、最後に一度だけ端数処理（切り捨て・四捨五入）を行う」** という日本の会計ルールに対応するため、取引ヘッダーに `tax_adjustment_jpy` を持ち、1円単位の完全な整合性を保証します。
...
2. **Inventory**: 同じ「じゃがいも」でも、「先週買った北海道産」と「昨日買った特売品」を別ロットとして管理。
3. **Meal**: 料理をする際は、特定の在庫ロットを引き当てて消費。
4. **Nutrition**: 食べたものだけが栄養として計上され、腐らせて捨てたもの（LOSS）はコストとしてのみ計上。

## 🏗 System Architecture

本システムは SQLite 上に構築されており、大きく5つの分類で構成されています。
